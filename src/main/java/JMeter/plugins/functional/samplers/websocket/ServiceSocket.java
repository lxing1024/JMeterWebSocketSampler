...
public class ServiceSocket {
    ...
    public void onMessage(byte buf[],int offset, int length) {
        synchronized (parent) {
            log.debug("Received message type is byte[]");
            String msg = Arrays.toString(buf);
            String length_temp = " (" + msg.length() + " bytes)";
            logMessage.append(" - Received message #").append(messageCounter).append(length_temp);
            addResponseMessage("[Message " + (messageCounter++) + "]\n" + msg + "\n\n");
    ...
    public void sendByteMessage(ByteBuffer message) throws IOException {

            try {
                //byte[] temp=hexStringToByteArray(message);
                //log.info("temp string-----"+temp.toString());
                //ByteBuffer buf = ByteBuffer.wrap(temp);
                //log.info("ByteBuffer string-----"+buf.toString());

                session.getRemote().sendBytes(message, new WriteCallback() {
                    @Override
                    public void writeFailed(Throwable throwable) {
                        log.info("write ByteBuffer failed-----");
                        logMessage.append("write ByteBuffer failed-----");
                    }

                    @Override
                    public void writeSuccess() {
                        log.info("write ByteBuffer success-----");
                        logMessage.append("write ByteBuffer success-----");
                    }
                });

            } catch (Exception e) {
                logMessage.append(" - Send byte exception"+ e.toString()).append("\n");
            }
