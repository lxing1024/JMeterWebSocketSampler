...
public class WebSocketSampler extends AbstractSampler implements TestStateListener {
    ...
    private ServiceSocket getConnectionSocket() throws URISyntaxException, Exception {
        ...
        //request.setSubProtocols("text","binary");
        log.info("request.setSubProtocols---"+request.getSubProtocols().toString());
    ...
    public SampleResult sample(Entry entry) {
        ...
        try {
            ...
            if (!payloadMessage.isEmpty()) {
                ...
                if (getRequestType().equals("bin")) {
                    log.info("payloadMessage-----"+payloadMessage.toString());
                    ByteBuffer buf=ByteBuffer.wrap(hexStringToByteArray(payloadMessage.trim()));
                    log.info("bytebuffer-----"+buf.toString());
                    socket.sendByteMessage(buf);
                } else {
